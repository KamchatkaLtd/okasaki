...
object ZipperTree {
  ...
  case class Section[E](c: List[Tree[E]]) extends Tree[E]
  ...
  case class Loc[E](t: Tree[E], p: Path[E]) {
    ...
    def go_up: Loc[E] = p match {
      ...
      case Node(left, up, right) => Loc(Section(left.reverse ::: right), up)
    }

    def go_down: Loc[E] = t match {
      case Item(_) => failwith("down of item")
      case Section(t1 :: trees) => Loc(t1, Node(Nil, p, trees))
    ...
    def insert_down(t1: Tree[E]): Loc[E] = t match {
      case Item(_) => failwith("down of item")
      case Section(sons) => Loc(t1, Node(Nil, p, sons))
    }
