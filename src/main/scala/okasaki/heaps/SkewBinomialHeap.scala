...
class SkewBinomialHeap[E](val h: H[E] = Nil)
                         (implicit val ord: Ordering[E])
  extends Heap[E, SkewBinomialHeap[E]] {

  override def empty = new SkewBinomialHeap[E](Nil)
  ...
  override def insert(x: E) = new SkewBinomialHeap[E](h match {
    case t1 :: t2 :: rest if t1.r == t2.r => skewlink(x, t1, t2) :: rest
    case ts => Node(0, x, Nil, Nil) :: ts
  })

  override def merge(o: SkewBinomialHeap[E]) =
    new SkewBinomialHeap[E](mergeTrees(normalize(h), normalize(o.h)))
  ...
  override def findMin = removeMinTree(h)._1.x

  override def deleteMin = {
    val (Node(_, x, xs, ts1), ts2) = removeMinTree(h)
    val ts = mergeTrees(normalize(ts1.reverse), normalize(ts2))
    xs.foldLeft(new SkewBinomialHeap[E](ts))(_ insert _)
  }
}
