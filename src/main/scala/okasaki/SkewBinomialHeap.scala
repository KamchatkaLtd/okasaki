...
import okasaki.SkewBinomialHeap.{H, Node}
...
class SkewBinomialHeap[E](implicit val ord: Ordering[E]) extends Heap[E, H[E]] {
  ...
  override def isEmpty: (H[E]) => Boolean = _.isEmpty
  ...
  override def insert: (E, H[E]) => H[E] = {
    case (x, t1 :: t2 :: rest) if t1.r == t2.r => skewlink(x, t1, t2) :: rest
    case (x, ts) => Node(0, x, Nil, Nil) :: ts
  }

  override def merge: (H[E], H[E]) => H[E] = {
    case (ts1, ts2) => mergeTrees(normalize(ts1), normalize(ts2))
  }
  ...
  override def findMin: (H[E]) => E = h => removeMinTree(h)._1.x

  override def deleteMin: (H[E]) => H[E] = h => {
